	...
	render() {
		...
		return (
			<ul className="sidebar-collapse-groups list-unstyled">
				{propertyGroups.map(
					propertyGroup => {
						const iconActiveClassName = getCN(
							{
								'active': propertyGroup.propertyKey === propertyKey
							}
						);

						const criteriaClassName = getCN(
						...
						return (<li className={criteriaClassName} key={key}>
							<div className="sidebar-collapse-header-root" onClick={this._handleClick(key, active)}>
								<a className="sidebar-collapse-header d-flex justify-content-between">
									...
									<span className="collapse-icon">
										<ClayIcon className={iconActiveClassName} iconName="angle-right" />
									</span>
								</a>
							</div>
